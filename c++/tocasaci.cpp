...
void dfs(int i, int j){ // crio minha função de busca
	...
	for (int k = 0; k < 4 && !id; ++k) { // percorro todos os vizinhos
		...
		if (matriz[xi][xj] == 2) {// verifico se a posição tiver valor igual a 2 termino minha função
			id = true;
			return; // termino a função
		}
		else // caso contrario
			if( matriz[xi][xj] == 1  ){ // se a posição tiver valor igual a 1 , aplico a função no xi, xj
...
int main() // abro minha função principal
{
	...
	for (int j = 0; j < m; ++j) cin >> matriz[i][j]; // pego o valor da posição


	for (int i = 0; i < n; ++i) // percorro toda matriz fazendo a busca
	for (int j = 0; j < m; ++j) if (matriz[i][j] == 3) dfs(i,j);//  verifico se a posição i e j e igual a 3 e aplico a função se for verdadeiro	
