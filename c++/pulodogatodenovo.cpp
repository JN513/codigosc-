...
void bfs(int x, int y){ // crio minha função bfs
    ...
    while(!fila.empty()){ //olho se tem alguem na fila
        ...
        for (int j = -2; j <= 2; j++){
            ...
            if (verifica(xi,xj) && dist[xi][xj] == oo && grid[xi][xj] == 1){ //verifico xi e xj ta nos limites, olho se a posição ja não foi visitada, e olho se i e j e uma lajota preto
...
int main(){ //inicio minha função inicial{
    ...
    if (dist[n-1][m-1] == oo) cout << "-1"; //verifico se a ultima lajota for igual a infinito falo que e impossivel chegar nela, se for impossivel imprimo -1
