...
public:
    ...
    bool foi_acertado(int x, int y){
        if(x == this->x and y == this->y and vivo == true) {
            this->vivo = false; 
            return true;
        }
        else if (x == this->x and y == this->y and vivo == false) return false;
        else if (x != this->x or y!= this->y) return false;
    }
        ...
        for(int i = 0; i < T; i++){
            ...
            if(this->municao > 0){
                for(int j = 0; j < quantidade_inimigos; j++){
                    if(inimigos[j].foi_acertado(x, y)) this->pontos += 10;
                }
